...
def start_parking_session(user_id, lot_id, lot_name):
    """
    ...
    try:
        ...
        # Insert session
        cursor.execute("""
            INSERT INTO parking_sessions (user_id, lot_id, lot_name, start_time, end_time)
            VALUES (%s, %s, %s, NOW(), NULL)
        """, (user_id, lot_id, lot_name))
...
def get_active_session(user_id):
    ...
    try:
        cursor.execute("""
            SELECT session_id, user_id, lot_id, lot_name, start_time
            FROM parking_sessions
            WHERE user_id = %s AND end_time IS NULL
            LIMIT 1
        """, (user_id,))
