...
VALUES
    ('stu001', (SELECT lot_id FROM parking_lots WHERE polygon_id = 'fac_1'), 'CB', DATE_SUB(NOW(), INTERVAL 6 DAY), 35.00, 'Unpaid', 'Parked in a faculty-only lot during restricted hours.', 'Permit mismatch', NULL),
    ('stu002', (SELECT lot_id FROM parking_lots WHERE polygon_id = 'com_bl12'), 'CR', DATE_SUB(NOW(), INTERVAL 4 DAY), 25.00, 'Pending', 'Vehicle remained in a blue lot after posted window.', 'Zone violation', NULL),
