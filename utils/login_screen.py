...
class LoginScreen(Screen):
    ...
    def login(self, instance):
        if not self.username.text or not self.password.text:
            self.error.text = "Enter email and password"
            return
        ...
        permit_name = user.get('user_permit_name')
        if not permit_name and user["role"] == "admin":
            permit_name = "Admin Access"
        if not permit_name:
            permit_name = "No Permit Assigned"
